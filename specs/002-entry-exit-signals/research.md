...
**實測結果（2026-09-03，T028）**：用 `git archive` 匯出改動前版本，
兩版對同一批測試資料（3 檔）各跑一次 `run_module_d_review`，攔截
`twse_price_client._throttled_get` 與 finmind 請求函式計數。spy 刻意
**不拋例外**（回傳「查無資料」形狀）讓流程完整跑完，避免「第一次呼叫
就中斷」的假數字：
...
**實測結果（2026-09-03，T021）**：用 `git archive` 匯出改動前版本，
兩版對正式庫 `revenue_yoy_cache` 的 **39 檔既有資料**（唯讀連線）
各跑一次 `_growth_deceleration`：

| 指標 | 改動前 | 改動後 |
|---|---|---|
| 觀察期數 | 固定 3 期 | **平均 5.9 期**（3~6） |
| flagged 檔數 | 1 | 1 |
| **判斷結果改變的標的** | — | **0** |

新邏輯的趨勢分佈：上升 29／下滑 2／持平 5／資料不足 3。

**如何解讀「0 檔改變」**：機制確實變了（期數幾乎翻倍、判斷準則從
「嚴格遞減」換成「斜率＋中位數」），但在**今天這批資料**上兩種準則
恰好給出相同結論——39 檔裡只有 2 檔呈下滑趨勢，其中 1 檔（1785）
兩版都觸發，另 1 檔（3008，`-3%→11%→23%→42%→-10%→-10%`）因含負值
而正確地不算「成長趨緩」（轉負是另一個問題，沿用改動前的既有語意）。

這不代表改動無效：舊準則要求「連續 3 期嚴格遞減」，遇到
「先崩後穩」「下滑但中間有一期反彈」都會漏掉；新準則用 6 期趨勢判斷
更穩健，只是本批資料沒有這類案例。日後資料變化時兩者會分歧——
`tests/test_exit_signals.py::RevenueTrendTest::test_flat_when_recovering`
就是釘住這個差異的測試。
