...
- **2026-08-16 補記**：「CLI排程入口」不是只有指令碼，2026-08-03起已
  ...
  `poc/kb-mcp/README.md`都沒記過，一度在對話裡被誤判成「還沒排程」。
  教訓同上：開發完成／launchd部署／文件記錄是三件可能各自落後的事，
  接手前除了`git log`，`~/Library/LaunchAgents/`跟`launchctl list`也要
  查，不要只看程式碼有沒有寫。詳見`poc/kb-mcp/README.md`「模組D每日
  排程」節。
