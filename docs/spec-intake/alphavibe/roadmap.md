...
## 目前開發優先序（2026-09-02 PO 定案，Q-048）
...
| — | Phase 2 正式產品啟動 | **維持按住不動**——2026-08-18（Q-045）「先打磨到常用、使用率提升才談架構升級」的決定本次未翻案 |

PO 表示目標是完成前三項。順位 1 的成因是 2026-09-02 一次真實的分析事故
（人工拼湊多個唯讀工具做進場時機分析時，`get_revenue_yoy` 的
`yoy_growth` 原始小數被誤當百分比、少乘 100 倍，導致多檔持股成長判斷
方向顛倒，靠事後交叉核對原始 API 才抓到），完整決策理由見
clarification-log.md Q-048。
...
- **2026-08-16 記錄：「加碼計畫總額度」欄位缺失**（雲端session做PR-review
  ...
  `suggested_add_pct`（遞減式加碼 40/25/15/10/10%）定義是「這次加碼佔
  此股加碼計畫總額度的比例」，但**「加碼計畫總額度」本身在整個系統裡
  沒有任何地方定義或儲存**（`kb_store.py`／`review_engine.py`都沒有
  對應欄位；工具自己的docstring也承認這件事沒定義），導致PO實際想看
  的「加碼進度（已投入/計畫總額／完成幾成）」目前完全算不出來，只能
  手動心算。跟已列 Deferred 的 Q-039（Score量化評分/風險評分自動化）
  是相鄰但不同的缺口，不要混為一談——這項是「有沒有地方存這個數字」，
  不是「要不要自動打分」。
  待補：新增一個可讀寫、**可調整**（因為投資預算會變動，不是寫死）的
  「計畫總額度」欄位／工具（例如 `save_position_plan`/`get_position_plan`，
  或掛在 holdings/stance 底下的新欄位），供 `check_position_control`
  讀取算出真正的完成比例。過渡期做法：先用 `save_comment` 存一筆帶清楚
  標記的備註（如「計畫額度：2308 台達電 目標NT$xxx」）代替，等有正式
  欄位再遷移。這項屬於 `poc/kb-mcp` 程式碼變動（`review_engine.py`／
...
- ✅ **金額/財務數字小數位數統一**（PR #14，2026-08-24 同天追加，PO 臨時
  反饋，不屬於原始 16 項盤點範圍）：投資分頁上跟錢/財務相關的數字（股票
  現價、主題集中度佔比與合計市值、選股篩選/市場掃描的營收年增率/回檔幅度/
  超額跌幅/股利率/目前價、貼庫存匯入預覽的平均成本）統一改成小數點後兩位
  （原本分別是 1 位／整數／無格式化，不一致）。刻意不動選股篩選頁「策略
  門檻」敘述句（如「PEG<1 且回檔>=40%」）的格式化——那是條件說明文字不是
  逐檔數據，強制 2 位小數會讓文字變得不自然。
