...
# 刻意只收錄長期穩定、無爭議的正式成員——聯盟成員資格會變動（例如
# 併購導致的過渡期成員、退出協商中的成員），沒把握的寧可不收錄，
# 讓它落回「查無資料」而非給錯答案。原本的版本多收了韓亞航空（大韓
# 航空併購整合中，星空聯盟資格有變動疑慮）、深圳航空（僅
# Connect Partner，非正式會員）、中國東方航空與加魯達印尼航空
...
def _airline_alliance(name):
    ...
    **`AIRLINE_NO_ALLIANCE` 必須先查**：廉價航空常以主要航空公司的
    品牌名當作自己名稱的一部分（例如「捷星日本航空」內含「日本航空」
    四個字），若先查 `AIRLINE_ALLIANCES` 會誤判成該品牌所屬的聯盟。
    2026-09-24 在正式資料上實測發現這個問題——「捷星日本航空」（無
    聯盟的廉航）被誤標成「寰宇一家」（日本航空所屬），查證後才發現是
    包含比對的順序問題，不是資料錯誤。
    """
