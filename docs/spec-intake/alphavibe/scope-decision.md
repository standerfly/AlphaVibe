...
（粗活）；對外服務層 2026-08-21 由 Q-046 推翻原local-first判斷，改為
FastAPI＋React服務化架構（品牌名STND），詳見下方「STND 個人主控台
擴建」節。（SRC-001 §1/§2、SRC-009、Q-028/Q-029/Q-032、Q-034
2026-07-27評估結論維持local-first——**已於2026-08-21由Q-046推翻**）
...
- **策略引擎與每日PDCA（模組A-G，2026-07-27 新增，源自
  ...
  - **交易流水表（FR-056）與遞減式加碼比例自動計算：2026-07-27從下方
    ...
    次數歷史，一併移出）
- **STND 個人主控台擴建（2026-08-21，Q-046，新增，2026-08-22上線）**：
  推翻 Q-034（服務化與多供應商架構，見上方 Goal 段落原local-first敘述）
  與 Q-045（先打磨到常用才談架構升級）——`poc/kb-mcp`全面重寫為
  FastAPI（`app/`）＋React（`web/`）輕量前後端分離，既有模組A-G功能
  全數遷移（`/screen`、`/market-scan`、`/dashboard`系列、5個表單端點、
  MCP連接器），底層計算邏輯與資料層未重寫。同時擴展產品定位為「STND
  個人一站入口」多分頁主控台：
...
- ~~服務化與多供應商架構~~（FastAPI＋Agent Router＋自建聊天 UI，
  ...
  **2026-08-21 已由 Q-046 推翻，不再 Deferred**——`poc/kb-mcp`全面重寫為
