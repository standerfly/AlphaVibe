...
**Last Updated:** 2026-07-09（SRC-010 追溯性需求納入，Q-034~Q-036；基線接受狀態不變）
...
- **核心迴圈（2026-07-07 主軸重塑，SRC-009、Q-029）**：與 AI 討論各來源
  資訊並歸檔 $\rightarrow$ **分析類股資金輪動以確認市場主軸** $\rightarrow$ 從基本面浮現目標股（名單驅動＋哲學驅動，Q-030）$\rightarrow$ 估值討論
  （先基本面好、再談買賣價）$\rightarrow$ 進出決策 $\rightarrow$ 記錄預計/實際進出場（後行）。
...
- **v1 In-Scope**：手動貼入三來源＋YouTube（FR-023）、三層知識庫、
  ...
  （FR-019~021，Q-029/Q-030）**、**類股資金流追蹤 (FR-032~037)**、儀表板（總覽名單＞資訊流優先，FR-024/025、
  Q-031）、**追溯性快照包（FR-026~028、FR-030，Q-036）**、**持股快照
  （FR-029，Q-035 部分納入）**、每日報告（支援能力）、自選股、觸價提醒、
  立場衝突標記、前置清洗、
  **AI 對話式輸入（FR-015~018，Q-024 已於 2026-07-08 確認納入）**。
- **後行階段（in scope、非 v1 首發）**：交易紀錄——預計/實際進出場
  （FR-022，Q-029）；watchpoints 事件提醒（FR-031，Q-036）。
- **Out-of-Scope**：持倉追蹤之損益計算與部位管理（Q-010；Q-035 邊界修正：
  持股快照 FR-029 納入、損益/部位管理仍排除；FR-022 進出場紀錄亦不屬此
  排除，見 SRC-009 OQ-2）、自動爬取/n8n、照片自動解析、自動通知、
  多用戶管理、歷史對話自動匯入（Q-023）。
- **Deferred**：**全市場條件篩選選股（Q-030）**、**服務化與多供應商架構
  （FastAPI/Agent Router/自建 UI，Q-034：Phase 2 前以 1b 實測經驗評估）**、
  全域投資助理（Q-019 v1 不做；Q-027 於 2026-07-08 定案列 Deferred）、
  Docker 雲端部署（Q-032）、n8n 自動化、持倉損益、通知、Feedback Loop、
  多用戶權限、來源權重邏輯、對話歷史保存策略、LLM 用量邊界（Q-025）。
...
- FR-028 快照 diff：同一標的歷次分析快照對照（結論/風險/價格/估值變化）
  （SRC-010 §4.3；Q-036）

**I. 追溯性與快照（SRC-010，Q-035/Q-036 新增）**
- FR-026 分析快照：結論凍結 {價格、估值、驅動因素/下檔風險/後續關注點、
  框架版本、產出模型}，經使用者確認後寫入（SRC-010 §3.1）
- FR-027 來源引用：快照附引用來源 {url, title, retrieved_at, 摘要}；
  查證單檔單次 ≤3 查詢（SRC-010 §3.1/§6）
- FR-029 持股快照：截圖 Vision 解析（沿 FR-010）入庫 {code, shares,
  avg_cost, snapshot_date}；**不含損益追蹤介面**（Q-035，Q-010 邊界維持）
- FR-030 篩選框架引擎：可版本化 checklist（Layer 1 結構化延伸），
  screen 結果入快照並記框架版本（SRC-010 §3.4）
- FR-031 watchpoints 事件提醒（除權息/法說/月營收）——**後行 Phase 2**
  （SRC-010 §4.2）

**J. 類股資金流追蹤（新需求，2026-07-12）**
- FR-032 自動化 ETL：每日 16:00 自動從 TWSE (上市) 與 TPEx (上櫃) 抓取類股成交金額、比重與漲跌幅
- FR-033 指標計算：計算成交值與比重的 5MA、10MA、20MA，以判斷資金流向趨勢
- FR-034 輪動評分系統：建立 Scoring 機制（成交值 $\uparrow$、比重 $\uparrow$、法人買超 $\uparrow$ $\rightarrow$ 高分），產生每日類股強度排名
- FR-035 儀表板整合：在總覽名單頁 (FR-024) 增加「類股資金熱力圖」或「輪動排行表」
- FR-036 訊號通知：當某類股出現「資金顯著轉強」訊號時，透過 LINE 發送提醒
- FR-037 個股關聯：在個股分析時，自動顯示該股所屬類股的目前資金狀態
...
- **免責聲明**：系統輸出為研究輔助資訊、非投資建議；介面固定顯示此聲明
  （SRC-010 §6；Q-036）
