...
- **研究筆記（`docs/research/`）寫作慣例（2026-08-27新增，見
  `docs/spec-intake/pending-verification-list/`）：完成或補充研究筆記
  時，若內容裡出現「這個判斷現在還無法蓋棺論定，要等到某個未來時間點/
  事件發生才能驗證」這種句型（例：「NVIDIA下一次財報公布是驗證這個
  假設的關鍵時間點」），主動向使用者建議是否要登記為結構化的「待觀察
  項目」（透過 `save_pending_verification` MCP tool），不要只是寫進
  散文段落就結束——這正是這個功能存在的理由：光寫成散文，之後很容易
  沒人記得回頭查（連寫下「屆時應更新此節」提醒的人，事後都不一定會
  真的回去更新，見上述 pre-spec 案例）。這是協作習慣，不是程式自動化，
  使用者仍可以說不用登記。
