...
| 機票（2026-09-23 新增，持續擴充中） | `Flights.jsx`／`FlightTrackForm.jsx` | `app/routers/flights.py` | `poc/kb-mcp/flight_store.py`（獨立 `FlightStore`＋獨立 db `flights.db`）／`flight_scan_service.py`／`flight_search.py`／`scraper/`（Node + Playwright） | **005（掃描分頁）與 006（價格追蹤）已上線**：正式服務已套用，排程進入點 `poc/kb-mcp/flight_tracking_job.py` 與 `ops/launchd/com.alphavibe.flighttracking.plist`（每日 09:30 自動重掃、跌破目標價發 Telegram 含現況通知、資料過期時停止通知）。**007（天數區間化）2026-09-24 已正式上線**——`trip_days` 從單一固定整數升級為 `trip_days_min`／`trip_days_max` 區間，系統展開多天數選項比價，並新增組合數上限守衛；264 個單元測試＋smoke test 119 項全綠，已合併進 `function/alphavibe`，正式庫既有的 id=9 條件已遷移至 10～14 天並重新查價驗證（最低價從固定 12 天的 NT$47,154 降到 14 天的 NT$46,872，證實區間確實找到更划算的天數組合）。**008（單純來回搜尋）2026-09-25 已完成開發並正式上線**：多候選目的地比價、可選偏好轉機城市、通知標示觸發目的地、組合數上限守衛，新增 `roundtrip_track`／`roundtrip_scan_result` 獨立表（不與四段票共用 schema）；298 個機票單元測試全綠＋smoke test 全項 PASS，已合併進 `function/alphavibe`、正式服務已重啟套用，並用真實 API 端到端驗證（TPE↔AOJ 最低 NT$17,921，既有 id=9 四段票條件不受影響）；同日追加兩項補強並已上線：新增 `PATCH /api/flights/tracks/roundtrip/{id}` 可編輯頻率／目標價（原列為 Deferred），以及建立表單補上「出發地機場」輸入框（原隱性固定 TPE）與目的地自訂輸入（原僅能從 16 個預設城市選）。規格見 `specs/005-flight-scan-page/`／`specs/006-flight-price-tracking/`／`specs/007-trip-day-range/`／`specs/008-roundtrip-search/` |
