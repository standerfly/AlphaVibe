...
- `scripts/prespec_init.py [feature-slug] --po "..." --tpm "..." [--title "..."]`
...
- `scripts/prespec_sync_index.py [feature-slug]` synchronizes the global
  ...
  feature workspaces.
- `scripts/prespec_status.py [feature-slug]` reports the current ADR-0027 step,
...
- `scripts/prespec_validate.py [feature-slug]` checks mechanical readiness:
...
Self-contained means the input package carries the portable requirement content
needed by `speckit-specify`; it must not rely on the reader opening supporting
artifacts to discover required API, workflow, state, data, failure, observability, or
compatibility behavior. When `readiness-checks.md` marks a supporting artifact as
required, copy its essential product decisions into the input package as a concise
summary section, not as a raw dump.
...
Add dynamic supporting-artifact summary sections based on the feature traits marked
required in `readiness-checks.md`. Record the mapping in
`## Supporting Artifact Coverage`. Use these mappings:

- Multi-role, multi-system, or multi-step workflow ->
  `## Workflow And Call Ordering`.
- Async job, callback, event handling, or state transition ->
  `## Order State And Idempotency`.
- New or changed external/internal API behavior ->
  `## API Contract Summary` and `## Non-Call And No-Change Constraints`.
- Third-party or cross-system integration ->
  `## Integration, Timeout, Retry, And Data Mapping`.
- New or changed data lifecycle ->
  `## Data Model And Compatibility`.
- Permission, role, or approval behavior ->
  `## Permission And Approval Behavior`.
- Security, privacy, compliance, or audit concern ->
  `## Error Handling Matrix` and `## Observability And Recovery`.
- Import, export, or batch processing ->
  `## Validation And Partial Failure Policy`.
- High-risk, irreversible, payment, order, or control flow ->
  `## Order State And Idempotency` and `## Error Handling Matrix`.
- Operationally sensitive behavior ->
  `## Observability And Recovery`.

If a trait is N/A in `readiness-checks.md`, do not add the corresponding dynamic
section unless the feature still needs it for self-contained handoff clarity. N/A
rationale must remain in `readiness-checks.md`.
