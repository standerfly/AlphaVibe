...
**修訂記錄（Accepted後的追加修訂，非推翻驗收）**：2026-09-06，`/speckit.clarify`
階段（`specs/003-us-stocks/`）PO 對 Q-001 做出第三輪決定，**推翻**了本文件
原本「不做背景排程輪詢」的決定，改為「每日一次背景排程自動刷新（比照
`market_scan.py` 先例），額度用盡時未刷新的股票顯示『未更新（無額度）』」。
本文件已同步更新對應章節（見 §Workflow、§Integration Note、§Constraints、
§Error Handling Requirements、§MVP Scope）。完整問答記錄見
`clarification-log.md` Q-001 第三輪；技術規格層級的完整定義見
`specs/003-us-stocks/spec.md` FR-006/007/011/013/017。維持 Status: Accepted
（驗收本身沒有被推翻，只有Q-001這一項決定被追加修訂）。
...
- 報價/基本面資料改採免費API多來源取得（主要：FMP；備援：候選Alpha Vantage或
  yfinance，待speckit-plan定案），**每日一次背景排程自動刷新**（比照
  `market_scan.py` 先例；Q-001第三輪決定，推翻第二輪「按需查詢非排程」，
  見§Integration Note）
...
- 每日一次以外的排程頻率（例如每小時刷新）——Q-001第三輪只定案「每日一次」
- 美股與台股資料的任何跨市場彙總計算
- 多使用者/多角色權限設計
...
- FR-03 個股詳情頁提供股價走勢圖，疊加買賣點位標記；視覺呈現比照台股既有
  ...
  股價資料由每日一次背景排程呼叫免費報價API（主要來源FMP）取得，可拿到
  FMP 免費層涵蓋範圍內的連續日K歷史股價（僅EOD收盤資料，非即時報價），
  非使用者手動輸入的離散快照（見 §Integration Note，Q-001第三輪決定）
...
- FR-06 關注條件清單：可為每檔股票設定監控門檻（例：股價、毛利率、NRR等指標），
  由每日一次的背景排程呼叫報價/基本面 API 取得最新資料並比對是否碰觸條件
  （比照`market_scan.py`先例，見 §Integration Note），碰觸時在美股分頁與
  個股詳情頁標示警示狀態（未觸發/已觸發/資料不足 三態）；NRR等SaaS特有
  指標多半查不到，仍需使用者/agent互動時手動補充，非API可自動取得
- FR-10 當日排程因外部資料源額度上限而未能刷新部分股票時，這些股票標示為
  「未更新（無額度）」，並保留其上一次成功取得的資料與監控條件評估結果；
  「未更新（無額度）」與「資料不足」是兩種不同狀態，前者代表「曾經有資料、
  當日沒刷到」，後者代表「從未成功取得過資料」，不得互相誤判
...
**循環 C（持續監控）**：每日一次背景排程（比照`market_scan.py`先例）呼叫
免費報價API（主要FMP，必要時切換備援來源）取得最新資料（見 §Integration
Note）→ 比對關注清單的觸發條件（FR-06）→ 碰觸條件時標記警示（狀態：未觸發
ok → 已觸發 alert，資料不足 pending，或額度用盡時的未更新 FR-10）→ 透過
既有 Telegram 閘道推播通知使用者（FR-09，Q-002）→ 使用者看到警示 → 回到
循環 A 重新討論是否調整立場/交易
...
- **關注條件**：代號、指標類型（價格/估值/基本面等）、比較條件、門檻值、
  狀態（未觸發/已觸發/資料不足）
- **美股報價歷史**：代號、日期、收盤價；資料來源為每日一次背景排程呼叫
  免費報價API（主要FMP）取得並存入（Q-001第三輪決定，比照`market_scan.py`
  先例）；若當日因額度用盡而跳過某股，當天不會新增紀錄，歷史可能有缺口
  （該股顯示「未更新（無額度）」，見FR-10）
...
**排程頻率（Q-001第三輪決定，2026-09-06）**：**每日一次**，比照本 repo
既有台股 `market_scan.py` 的排程模式（收盤後執行）。此決定推翻了 Q-001
第二輪原本「使用者/agent互動時按需查詢，非背景排程」的決定——PO 在
`/speckit.clarify` 階段明確要求改為排程自動刷新。確切執行時刻、額度如何
分配給多檔追蹤股票（依優先序或依序執行至額度用盡）屬技術實作細節，留待
speckit-plan。

**失敗/降級行為**：分兩種情況——(1) 某股票從未成功取得過任何資料（主要
來源查無此股，或主備來源都查詢失敗）：關注條件顯示「資料不足」（pending，
見 FR-06 三態設計）。(2) 當日排程因額度上限而**跳過**某股票的刷新（該股
過去曾有資料）：顯示「未更新（無額度）」（FR-10），保留上一次成功取得的
資料與監控條件評估結果，不清空、不誤判為「資料不足」。兩者皆不阻塞其他
功能。可參照本 repo 既有 `benchmark.py` 對 FinMind 額度用盡時的優雅降級
先例（顯示異常提示、不影響其他欄位）。

**明確排除**：每日一次以外的排程頻率（例如每小時刷新）——250次/日的免費
額度撐得住「每日一次」，但更高頻率的排程會有超額風險，MVP 只做每日一次。
...
3. **正常路徑－關注條件觸發**：每日排程取得某檔股票最新資料 → 系統發現
   股價跌破使用者設定的門檻 → 透過 Telegram 閘道推播通知使用者 → 美股
   landing 頁該檔列顯示「已觸發」狀態 → 使用者點入個股詳情頁看到具體是
   哪個條件觸發、目前數值與門檻的對比
...
5. **例外路徑－某股票從未成功取得過資料**：關注條件比對時若查無資料
   （FMP查無該股、且備援來源也失敗），狀態顯示為「資料不足」（pending，
   非「未觸發」），避免誤判為條件正常；不觸發推播
6. **例外路徑－當日排程額度用盡**：排程執行到一半碰觸FMP免費額度上限 →
   尚未刷新的股票顯示「未更新（無額度）」，保留上一次成功取得的資料與
   監控條件評估結果（FR-10）→ 不誤判為「資料不足」，也不因此觸發新推播
...
- 報價/基本面資料透過免費API多來源取得（主要FMP＋備援），**每日一次背景
  排程自動刷新**（比照`market_scan.py`先例）；僅用免費層，不升級付費方案；
  額度用盡時未刷新股票顯示「未更新（無額度）」，保留上次成功資料（Q-001
  第三輪決定，推翻第二輪的「按需查詢非排程」）
...
| 某股票從未成功取得過資料（例如冷門股票查不到、代號打錯） | 關注條件狀態顯示「資料不足」，不誤判為未觸發，不觸發推播 | 個股列表/詳情頁 pill 顯示 pending 灰色狀態 | 下次排程取得資料後自動轉為 ok/alert | No（已有明確產品行為，沿用 SRC-002 mockup 的三態設計） |
| 當日排程因額度上限跳過某股票的刷新（該股過去曾有資料） | 顯示「未更新（無額度）」（FR-10），保留上一次成功資料與監控條件評估結果，不改判為資料不足、不重複推播 | 個股列表/詳情頁顯示「未更新（無額度）」標記，監控狀態維持上次結果 | 隔天排程額度重置後自動刷新 | No（Q-001第三輪已定案此降級行為） |
