...
`specs/008-roundtrip-search/plan.md`（單純來回搜尋：多目的地候選、
...
前先讀 `specs/005-flight-scan-page/quickstart.md`——它列出「已完成不要
重寫」的既有能力與三個已發生過的事故坑。上游需求基線：
`docs/spec-intake/flight-search/`（Accepted，005/006 來源）、
`docs/spec-intake/flight-roundtrip-search/`（Accepted，007 與
roundtrip-search 來源）。
