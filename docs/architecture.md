...
| 機票（2026-09-23 新增） | `Flights.jsx`／`FlightTrackForm.jsx` | `app/routers/flights.py` | `poc/kb-mcp/flight_store.py`（獨立 `FlightStore`＋獨立 db `flights.db`）／`flight_scan_service.py`／`flight_search.py`／`scraper/`（Node + Playwright） | **尚未上線**——US1~US4 已完成並通過 176 個單元測試與 smoke test（含 30 併發與組合數深度比對），但尚未合併進 `function/alphavibe`、正式服務也還沒重啟套用；規格見 `specs/005-flight-scan-page/` |
