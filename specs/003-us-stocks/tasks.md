...
## ⚠️ 實作前必看的一個範圍判斷（非阻塞，但影響 Phase 3 任務怎麼寫）

`research.md` §3 查證發現：既有交易匯入工具全部**只接受文字，不接受圖片**，
辨識步驟發生在「Claude 讀圖轉文字」這一層，不是後端服務的工作。但
`spec.md` User Story 1 的 Acceptance Scenario 描述「使用者在美股分頁的
交易匯入畫面上傳截圖」，讀起來像是網頁表單直接上傳圖片。

**本任務清單採用的解讀**（比照既有系統的實際運作模式，非憑空假設）：
真正的「上傳截圖」動作發生在**與 Claude 的對話中**（使用者把截圖貼進
對話），Claude 讀圖轉文字後呼叫 `parse_and_save_us_trade` 存成待核對紀錄；
網頁的「交易匯入畫面」只負責**呈現核對結果、讓使用者確認/修正**（對應
mockup `Import.dc.html` 的 STEP 2），**不實作 STEP 1 的網頁拖曳上傳
UI**——那個 UI 元件本身無法在沒有 OCR 的前提下運作。

若這個解讀跟你設想的不一樣（例如你確實想要一個「上傳圖片到網頁、後端做
OCR」的流程），請在開始執行 Phase 3 任務前先喊停，這會改變 T013/T017
的做法。
