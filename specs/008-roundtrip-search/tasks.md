...
- [X] T001 執行 `.venv/bin/python3 -m unittest discover -s poc/kb-mcp/tests -p "test_flight*"`，確認目前 264 個機票測試全綠（基準線）
- [X] T002 **驗證 scraper 對「2 段來回」頁面的相容性**（research.md §1「尚待驗證」）：用 `flight_search.google_flights_url()` 手動組一個 2 段的來回網址（例如 TPE→AOJ 去程＋AOJ→TPE 回程），透過 `scrape_itineraries()` 或直接執行 `scraper/flight_scraper.js` 對該網址查價，確認能正確抓到價格、且 `isMultiCity` 判斷邏輯正確落入「非多城市」分支。**這是本 feature 風險最高的假設，必須在其他任務開始前驗證**——若不相容，需要回頭修改 scraper 而非在 US1 階段才發現
...
- [X] T003 在 `poc/kb-mcp/flight_store.py` 的 `SCHEMA` 新增
...
- [X] T004 在 `poc/kb-mcp/flight_store.py` 新增
...
- [X] T010 [US1] 在 `poc/kb-mcp/flight_scan_service.py` 新增
...
- [X] T013 [US1] 在 `app/routers/flights.py` 新增
      ...
      組 2 段網址
- [X] T014 [US1] 在 `app/routers/flights.py` 修改
      ...
      額外含 `destination` 欄位
- [X] T015 [US1] 在 `app/routers/flights.py` 新增
      `DELETE /api/flights/tracks/roundtrip/{id}`（contracts/roundtrip-api.md
      §5），行為比照既有四段票刪除端點
- [X] T016 [US1] 在 `app/routers/flights.py` 新增
...
- [X] T017 [P] [US1] 在 `web/src/pages/FlightTrackForm.jsx` 新增
      單純來回建立表單：行程類型切換（四段票／單純來回）、候選目的地
      多選輸入、天數區間（沿用 007 的兩輸入框模式）、出發區間
- [X] T018 [P] [US1] 在 `web/src/pages/Flights.jsx` 新增類型切換 UI；
...
- [X] T019 [US1] 在 `app/tests/test_smoke.py` 新增單純來回的深度比對
...
- [X] T020 [US2] 擴充 `poc/kb-mcp/flight_scan_service.py` 的
...
- [X] T021 [US2] 在 `poc/kb-mcp/tests/test_flight_scan_service.py`
...
- [X] T022 [US2] 在 `app/tests/test_smoke.py` 新增指定 `preferred_transit`
...
- [X] T023 [US3] 在 `poc/kb-mcp/flight_scan_service.py` 新增
...
- [X] T024 [US3] 在 `poc/kb-mcp/flight_tracking_job.py` 的排程迴圈
...
- [X] T025 [US3] 在 `poc/kb-mcp/tests/test_flight_tracking.py` 補上
      測試：`build_roundtrip_notification()` 內容含候選目的地名稱；
      排程迴圈正確涵蓋兩種類型的到期條件；多候選目的地情境下通知
      判定基準是全部候選目的地中的最低價（spec.md User Story 3
      情境 3／Edge Cases）
- [X] T026 [US3] 在 `poc/kb-mcp/tests/test_flight_scan_service.py`
...
- [X] T027 [US4] 在 `app/routers/flights.py` 的
...
- [X] T028 [P] [US4] 在 `app/tests/test_smoke.py` 新增單純來回組合數
...
- [X] T029 完整回歸：`poc/kb-mcp/tests` 全套綠、`npx vite build`
      （`web/`）通過、smoke test 乾淨測試庫整體 PASS
- [X] T030 [P] 更新 `CLAUDE.md`／`docs/architecture.md` 的機票分頁列，
      補上單純來回搜尋已上線的現況
- [X] T031 更新
