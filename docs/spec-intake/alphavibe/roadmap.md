...
| **1h. 研究啟動包（新規劃，2026-08-10）** | 候選FR-060：`prepare_research_brief(code, peers=None)`——依SRC-012 checklist骨架機械蒐集財務數字＋排版，判斷性欄位留白待對話討論（方向A，見SRC-013） | ✅ Stage 1/2/3全數完成（2026-08-11）；Stage 4視使用狀況再評估，非阻塞 | `raw/src-013-research-brief-tool-design.md`；`research_brief.py`；測試637/637綠（主對話獨立重跑確認） |
...
- **2026-08-10｜外部研究流程框架與 FR-051/FR-021 對照討論（未定案）**：
  PO提供的Substack文章與AlphaVibe現況比對，找出「預期差」「業務理解/
  護城河」「現金流健康度」三個FR-051潛在補強方向，以及重申「不要把
  情境估值做成結構化目標價欄位」（FR-021已有前車之鑑）——完整討論見
  `raw/src-012-substack-research-process-checklist.md`。**尚未執行任何
  一項**，是否要動FR-051或新增Layer 1哲學文件待PO裁決。
...
### 1h 研究啟動包（規劃中，2026-08-10 PO確認要做）
- **背景與設計依據**：完整討論見 `raw/src-013-research-brief-tool-design.md`
  （延續SRC-012）。核心判斷：文章checklist七節裡，只有「財務體檢」多數
  可自動查數字，其餘六節（業務理解/產業結構/預期差/破裂條件/估值敘事/
  收斂四問）本質是判斷，不能自動生成——已評估並排除「全自動產出報告」
  方向（幻覺風險＋違背FR-021/FR-026「自動結構化結論0%使用」的既有教訓），
  採用「機械蒐集＋排版，判斷性欄位留白待對話討論」的方向。
- **候選FR-060**，尚未走scope-decision正式流程，product-spec.md暫不動。
- **分階段計畫**（逐步完成，每階段接手前先讀SRC-013確認最新進度，不要
  只看這裡的摘要）：
  1. Stage 1（MVP）：新增唯讀MCP工具`prepare_research_brief(code)`，
     純組裝現有工具（`get_fundamentals`／`get_revenue_yoy`／
     `get_institutional_trading`／`get_stock_price_history`等，實際
     涵蓋範圍以查證為準）輸出，依checklist財務六面向骨架排版，其餘
     六節固定佔位「〔需要對話討論〕」。不呼叫LLM生成任何敘事。
  2. Stage 2：支援對照組股票並排交叉驗證——**待PO裁決**對照組怎麼決定
     （沿用FR-041主題標籤自動抓，或PO手動指定）。
  3. Stage 3：與對話流程掛勾的觸發時機——**待PO裁決**是PO主動觸發，
     還是模組C篩出新候選時併入FR-057排程自動先跑一次。
  4. Stage 4：視Stage 1-3實際使用狀況，再評估要不要讓Claude主動依
     啟動包＋SRC-012 checklist哲學文件引導逐項討論。
- **已知資料邊界（誠實列出，任何Stage都解決不了）**：法說會Q&A逐字稿、
  TAM市場規模、法人共識預期/預估修正方向——台股無免費結構化API，永遠
  留白，需PO自己讀到的外部資訊補進對話。
- **現況（2026-08-11）**：Stage 1/2/3全數完成。
  - Stage 1（MVP）：`research_brief.py`（`prepare_research_brief()`）＋
    `finmind_client.get_balance_sheet_summary()`＋兩個新MCP工具
  - Stage 2（對照組）：PO裁決「列候選清單交PO判斷，系統不自動挑」。
    `prepare_research_brief(code, peers=None)`——未指定peers時列同產業
    候選（`peer_candidates`，一次`get_stock_info`全量查詢不逐檔耗額度）；
    指定peers時並排財務體檢五項（`peer_comparison`），互斥不評語
  - Stage 3（觸發）：PO裁決「PO手動觸發」。呈現規則寫進
    `supporting-artifacts/2026-08-10-framework-pre-buy-research-checklist-draft.md`；
    `prepare_research_brief`／`get_balance_sheet`已加入
    `server_readonly.py`白名單
  - 測試637/637，主對話獨立重跑驗證＋用真實2330（含peers=["2303"]情境）
    核對過輸出。完整證據見SRC-013「Stage 1/2/3 完成紀錄」小節
  - Stage 4（視使用狀況再評估要不要讓Claude主動引導逐項討論）尚未啟動，
    非阻塞，待實際用過Stage 1-3再看
