...
| 機票（2026-09-23 新增，持續擴充中） | `Flights.jsx`／`FlightTrackForm.jsx` | `app/routers/flights.py` | `poc/kb-mcp/flight_store.py`（獨立 `FlightStore`＋獨立 db `flights.db`）／`flight_scan_service.py`／`flight_search.py`／`scraper/`（Node + Playwright） | **005（掃描分頁）與 006（價格追蹤）已上線**：正式服務已套用，排程進入點 `poc/kb-mcp/flight_tracking_job.py` 與 `ops/launchd/com.alphavibe.flighttracking.plist`（每日 09:30 自動重掃、跌破目標價發 Telegram 含現況通知、資料過期時停止通知）。**007（天數區間化）開發完成待部署**——`trip_days` 從單一固定整數升級為 `trip_days_min`／`trip_days_max` 區間，系統展開多天數選項比價，並新增組合數上限守衛；264 個單元測試＋smoke test 119 項全綠，遷移腳本 `poc/kb-mcp/migrate_trip_days_range.py` 已備妥，尚未合併進 `function/alphavibe`、正式庫既有的 id=9 條件尚未遷移；規格見 `specs/005-flight-scan-page/`／`specs/006-flight-price-tracking/`／`specs/007-trip-day-range/` |
